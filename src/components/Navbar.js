...
// import {laptop} from "../data/data.json"
...
const Navbar = ({setQuery,query}) => {
  return (
    <div className="nav">
    <div className="logo">
    {/* Link tag is similar to anchor tag and to is same as href. But Link tag is used because page not get loaded
    in case of link tag*/}
             <Link to="/">
    ...
    <Link className="navlinks" to="/miphones">Mi Phones</Link>
           ...
           <div className='searchbox'>
               <input type="text" placeholder="Search Product" onChange={e=>setQuery(e.target.value)} value={query}></input>
               {searchIcon}
